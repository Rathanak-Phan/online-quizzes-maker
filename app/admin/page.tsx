...
export default function AdminPage() {
  ...
  // Fetch stats from API at runtime
  useEffect(() => {
    fetch("/api/admin/stats")
      .then((res) => res.json())
      .then(setStats)
      .catch(() =>
        setStats({ totalUsers: 0, totalTeachers: 0, pendingTeachers: 0, totalQuizzes: 0 })
      );
  }, []);
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-xl shadow-lg p-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Link
            href="/admin/users"
            className="bg-blue-600 text-white py-4 rounded-lg text-center hover:bg-blue-700 transition font-medium"
          >
            Manage Users
          </Link>
          <Link
            href="/admin/quizzes"
            className="bg-green-600 text-white py-4 rounded-lg text-center hover:bg-green-700 transition font-medium"
          >
            View Quizzes
          </Link>
          <Link
            href="/admin/reports"
            className="bg-purple-600 text-white py-4 rounded-lg text-center hover:bg-purple-700 transition font-medium"
          >
            Reports
          </Link>
          <Link
            href="/admin/settings"
            className="bg-gray-700 text-white py-4 rounded-lg text-center hover:bg-gray-800 transition font-medium"
          >
            Settings
          </Link>
        </div>
